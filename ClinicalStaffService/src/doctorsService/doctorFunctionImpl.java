...
public class doctorFunctionImpl implements doctorFunctions {
	...
	public ResultSet getRandevous(int doctor_id) {
		...
		try {
			...
			String query = "SELECT * FROM `randevou`, `patients` WHERE randevou.Patient_ID = patients.Patient_ID and randevou.Doctor_ID= "
					+ doctor_id
					+ " and  randevou.Date = CURDATE() and patients.Dead=0;";
	...
	public ResultSet getRandevouInfo(int randevou_id) {
		...
		try {
			connectDB connection = new connectDB();
			String query = "SELECT * FROM `randevou`, `patients` WHERE randevou.Patient_ID = patients.Patient_ID and randevou.Randevou_ID = "
					+ randevou_id + " ;";
			connection.resSet = connection.stmt.executeQuery(query);
			return connection.resSet;
		} catch (SQLException se) {
	...
	public boolean updateRandevou(int randevou_id, String Condition,
			int Medication, String comments, int overule, int lm, int dead,
			int sh, int patient_id) {
		// TODO Auto-generated method stub
		try {
			connectDB connection = new connectDB();
			String query1 = "UPDATE randevou SET  Medication_ID = "
					+ Medication + ", Conditions = '" + Condition
					+ "', Comments = '" + comments + "', Overule = " + overule
					+ " , Updated = " + 1 + " WHERE Randevou_ID = "
					+ randevou_id + ";";
			System.out.println(query1);
			connection.stmt.executeUpdate(query1);

			String query2 = "UPDATE patients SET Madness_level = " + lm
					+ ", Dead = " + dead + ", Self_harm = " + sh
	...
	public ResultSet getNotUpdatedRandevous(int doctor_id) {
		...
		try {
			connectDB connection = new connectDB();
			String query = "SELECT * FROM `randevou`, `patients` WHERE randevou.Patient_ID = patients.Patient_ID and randevou.Append=1 and randevou.Updated = 0 and randevou.Doctor_ID= "
					+ doctor_id
					+ " and CURDATE() > randevou.date and patients.Dead=0;";
			connection.resSet = connection.stmt.executeQuery(query);
	...
	@Override
	public boolean insertIncentend(int patient_id, String details,
			int medication) {
		// TODO Auto-generated method stub
		try {
			connectDB connection = new connectDB();
	...
	public String checkForAllergies(int p, int m) {
		try {
			connectDB connection = new connectDB();
			String query = "SELECT * FROM `allergies` WHERE allergies.Patient_ID = "
					+ p + " and allergies.Medication_ID= " + m + " ;";
			connection.resSet = connection.stmt.executeQuery(query);
			System.out.println(query);
	...
	public String[] getDrugInfo(int dr_id) {
		try {
			connectDB connection = new connectDB();
			String query = "SELECT * FROM `drugs` WHERE drugs.Medication_ID = "
					+ dr_id + ";";
			connection.resSet = connection.stmt.executeQuery(query);
	...
	public boolean updateDropIn(int randevou, int d_id) {
		try {
			connectDB connection = new connectDB();
			String query1 = "UPDATE randevou SET  drop_in = 1, Drop_in_date = NOW() ,Drop_in_doc = "
					+ d_id + " WHERE Randevou_ID = " + randevou + ";";
			System.out.println(query1);
			connection.stmt.executeUpdate(query1);
