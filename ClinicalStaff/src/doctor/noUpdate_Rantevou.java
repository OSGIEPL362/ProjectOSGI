...
public class noUpdate_Rantevou extends JFrame {
	...
	public noUpdate_Rantevou(int doctor_id) {
		...
		panel.setBorder(new TitledBorder(
				new LineBorder(new Color(0, 0, 128), 3), "Randevou List",
		...
		panel_1.setBounds(26, 30, 515, 354);
		panel.add(panel_1);
		...
		String header[] = new String[] { "Randevou ID", "Patient Name", "Date",
				"Time" };
		...
		try {
			while (rs.next()) {
				...
				date.add(date1);
				String time1 = rs.getString("Time") + ".00";
				time.add(time1);
		...
		btnGoGoCoco.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (s.matches("[0-9]+")) {
					...
					if (l.contains(no)) {
						patient_record_gui frame = new patient_record_gui(
								doctor_id, no);
						...
						JOptionPane.showMessageDialog(null,
								"Erron! error input", "Insert error",
		...
		btnBack.setBounds(352, 320, 101, 23);
		panel_1.add(btnBack);
	}
}
